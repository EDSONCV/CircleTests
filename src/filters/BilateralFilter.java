...
public class BilateralFilter implements ImageFilter {
    // SigmaColor:How similar are the neighboring colors needed to be in order to be mixed?
    private OptParam sigmaColor = new OptParam("Bi_SigColor", 75, 10, 150, 5, false);
    // SigmaSpace: distance of the pixels to be considered (similar to the Gaussian Kernel)
    private OptParam sigmaSpace = new OptParam("Bi_SigSpace", 75, 10, 150, 5, false);
    // Diameter: Pixel neighborhood diameter (similar to kernel size)
    private OptParam diameter = new OptParam("Bi_Dia", 9, 1, 15, 2, true);

    @Override
    public Mat process(Mat input) {
        Mat output = new Mat();
        // The bilateral filter requires an 8-bit input image.
